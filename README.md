...
> 关于飞轮，诚实说明：信任分靠 agent 在验证项目后调 `loom_record_outcome(refs, success)`
> 累积——这是**半自动**的，需要 agent 主动反馈（当前不强制、也无法从"文件被写盘"自动推断
> "代码真好用"）。不反馈时信任分停在初始值，检索退化为纯语义排序。飞轮机制本身
> （能升能降、Beta-Bernoulli）已验证，但"越用越强"依赖 agent 配合回报。
